...
readConfig() {
	...
	SS_SET_BACKLIGHT=$(fgrep "SS_SET_BACKLIGHT=" $SS_CONFIG | sed 's/SS_SET_BACKLIGHT=//')
	SS_SET_FRAMEBUFFER=$(fgrep "SS_SET_FRAMEBUFFER=" $SS_CONFIG | sed 's/SS_SET_FRAMEBUFFER=//')
...
dumpConfig() {
	...
	echo "<1>SS_SET_BACKLIGHT=$SS_SET_BACKLIGHT" > /dev/kmsg
	echo "<1>SS_SET_FRAMEBUFFER=$SS_SET_FRAMEBUFFER" > /dev/kmsg
